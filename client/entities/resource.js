(function(){
  ...
  US.createResource=function(type,x,y,uid){
    ...
    return {uid:uid||US.uid('res'),kind:'resource',type,cfg,x,y,r:24,amount:US.irand(cfg.amount[0],cfg.amount[1]),cooldown:0,
      update(dt){if(this.amount<=0){this.cooldown-=dt;if(this.cooldown<=0){this.amount=US.irand(cfg.amount[0],cfg.amount[1]);}}}
    };
  };
