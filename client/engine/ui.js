...
${completedIds.length ? '<div style=\"color:#64748b; font-size:11px; margin-top:8px; margin-bottom:4px;\">COMPLETED</div>' + completedBadges : ''}`;
         ...
         * The buy handler is exposed on window so inline onclick can reach it safely.
         */
        _renderGE() {
            const D = window.Duskfall;
            const g = this._game();

            // Expose the buy handler on window so the onclick attribute can call it
            // without closures that break when innerHTML is replaced.
            window._unkGEBuy = (itemId, price, label) => {
                ...
                inv.add(itemId, 1);
                game.ui.toast('Purchased!', `${label} bought for ${price} GP.`, 'good');
                game.ui.log(`Grand Exchange: bought ${label} for ${price} GP.`, 'gold');
                // Refresh so gold balance updates immediately
                this.refreshActiveTab();
            };

            const coins = this._coins();

            // GE catalogue — pulled from D.PRICES where item exists in D.ITEMS
            const catalogue = [
  ...
  <button style="${btnStyle}" onclick="window._unkGEBuy('${item.id}', ${item.price}, '${item.label.replace(/'/g, '')}')">${canAfford ? 'BUY' : 'NO GP'}</button>
</div>`;
            }).join('');
...
${rows}`;
         ...
         * Called by: switchTab(), init(), setInterval (1 s), _unkGEBuy()
         * NOT called from the render loop — zero frame-rate impact.
         */
        refreshActiveTab() {
