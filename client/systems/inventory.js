...
const US = window.UnkScape = window.Duskfall;
  ...
  US.InventorySystem.prototype.remove=function(id,qty=1){
    if((this.items[id]||0)<qty)return false;
    this.items[id]-=qty;if(this.items[id]<=0)delete this.items[id];return true;
  };
  US.InventorySystem.prototype.count=function(id){return this.items[id]||0};
  US.InventorySystem.prototype.has=function(id,qty=1){return this.count(id)>=qty};
  US.InventorySystem.prototype.canAfford=function(req){return Object.entries(req||{}).every(([id,q])=>this.has(id,q));};
  US.InventorySystem.prototype.take=function(req){if(!this.canAfford(req))return false;Object.entries(req).forEach(([id,q])=>this.remove(id,q));return true;};
  US.InventorySystem.prototype.quickUse=function(id){if(this.has(id))this.use(id);else this.game.ui.toast('Missing item',`You do not have ${US.ITEMS[id]?.name||id}.`,'bad');};
  US.InventorySystem.prototype.useHotbar=function(index){
    ...
    if(!this.has(id) && !Object.values(g.player.equipment||{}).includes(id)){
      g.ui.toast('Missing Hotbar Item',`Slot ${index+1}: ${US.ITEMS[id]?.name||id} is not in your backpack.`, 'bad');
      return false;
    }
    const equipped=Object.entries(g.player.equipment||{}).find(([slot,item])=>item===id);
    if(equipped){g.ui.toast('Selected',`${US.ITEMS[id]?.name||id} is already equipped.`, 'good');return true;}
    return this.use(id);
  };
  US.InventorySystem.prototype.use=function(id){
    const it=US.ITEMS[id], p=this.game.player;if(!it||!this.has(id))return false;
    if(it.type==='food'){
  ...
  US.InventorySystem.prototype.equip=function(id){
    const it=US.ITEMS[id], p=this.game.player;if(!it||!it.slot||!this.has(id))return false;
  ...
  US.InventorySystem.prototype.unequip=function(slot){
  ...
  US.InventorySystem.prototype.toSave=function(){return {...this.items};};
  US.InventorySystem.prototype.fromSave=function(data){this.items={...(data||{})};};
})();
