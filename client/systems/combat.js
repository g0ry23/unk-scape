(function(){
  ...
  US.CombatSystem.prototype.weaponStyle=function(player){
    ...
    return US.EQUIPMENT[w]?.style || US.ITEMS[w]?.combatStyle || 'melee';
  };
  US.CombatSystem.prototype.rollBonusHit=function(player,enemy,heavy=false){
    const g=this.game, style=this.weaponStyle(player), aimX=g.input.mouse.worldX, aimY=g.input.mouse.worldY;
    const combatLevel=US.levelForXp(player.skills.combat?.xp||0);
  ...
  US.CombatSystem.prototype.releasePrimary=function(held){
    const heavy=held>=this.heavyThreshold;
    this.playerAttack(heavy, Math.min(held,this.maxCharge));
  };
  US.CombatSystem.prototype.playerAttack=function(heavy=false,charge=0){
    ...
    const chargeBonus=US.clamp(charge/this.maxCharge,0,1);
    ...
    const accuracy=US.clamp(stats.accuracy-(heavy ? 0.08 : 0),.1,.96);
    ...
    let targets=g.entities.enemies.filter(e=>!e.dead&&US.dist(p,e)<range+e.r);
    if(style!=='melee'){
    ...
    targets.sort((a,b)=>{
      ...
      const aPlayer=US.dist(p,a), bPlayer=US.dist(p,b);
    ...
    for(const e of targets.slice(0,maxTargets)){
      ...
      if(hit){
        e.hitMarker=.2;
        e.hitFlash=.16;
        const lvl=US.levelForXp(p.skills.combat.xp);
  ...
  US.CombatSystem.prototype.distanceToAimLine=function(p,e,aimX,aimY){
    const ax=p.x, ay=p.y, bx=aimX, by=aimY;
    const dx=bx-ax, dy=by-ay, len2=dx*dx+dy*dy;
    if(len2<1)return Math.hypot(e.x-ax,e.y-ay);
    const t=US.clamp(((e.x-ax)*dx+(e.y-ay)*dy)/len2,0,1);
    const px=ax+t*dx, py=ay+t*dy;
    return Math.hypot(e.x-px,e.y-py);
  };
  US.CombatSystem.prototype.isEnemyNearAimLine=function(p,e,aimX,aimY,range,heavy=false){
    const dist=US.dist(p,e); if(dist>range+e.r)return false;
  ...
  US.CombatSystem.prototype.spawnAttackProjectile=function(p,tx,ty,hitPower,style,heavy,target,hit){
    const g=this.game;
    ...
    const proj=US.createProjectile(startX,startY,tx,ty,{speed,r,color,life:1.0,damage:hitPower||0,source:'player',recoverable:style==='range',recoverChance:heavy?.82:.68,dropId:'arrow',style});
  ...
  US.CombatSystem.prototype.enemyAttack=function(e){
    ...
    if(hit)p.takeDamage(US.irand(1,Math.max(1,e.cfg.attack)),e);else g.ui.floatText(p.x,p.y-28,'MISS','#9aa8c7');
  };
  US.CombatSystem.prototype.kill=function(e){
    const g=this.game;
  ...
  US.CombatSystem.prototype.update=function(dt){
    ...
    const aimX=g.input.mouse.worldX, aimY=g.input.mouse.worldY;
    const near=g.entities.enemies.filter(e=>!e.dead&&US.dist(p,e)<150).sort((a,b)=>{
      const aAim=Math.hypot(a.x-aimX,a.y-aimY), bAim=Math.hypot(b.x-aimX,b.y-aimY);
      const aPlayer=US.dist(p,a), bPlayer=US.dist(p,b);
