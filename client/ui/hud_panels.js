...
US.HudPanels = {
  ...
  init: function(){
    ...
    if (invHeader && !invHeader.querySelector('.unk-collapse-btn')){
      ...
      bL.className='unk-collapse-btn'; bL.title='Collapse hub (B)'; bL.textContent='-'; bL.style.marginLeft='6px';
    ...
    if (radHeader && !radHeader.querySelector('.unk-collapse-btn')){
      ...
      bR.className='unk-collapse-btn'; bR.title='Collapse chat (C)'; bR.textContent='-';
...
// Wheel: no modifier = hotbar scroll; modifier held = camera zoom (input.js handles it)
// Does NOT stopPropagation so camera zoom still works.
...
// B = toggle hub (left col), C = toggle chat (right col)
// Only during play, skips input/textarea targets.
window.addEventListener('keydown',function(e){
  var g=US.game; if(!g||g.state!=='play') return;
  var tag=(e.target&&e.target.tagName||'').toLowerCase();
  if(tag==='input'||tag==='textarea') return;
  var k=(e.key||'').toLowerCase();
  if(k==='c'){ US.HudPanels.toggleChat(); e.preventDefault(); }
  else if(k==='b'){ US.HudPanels.toggleHub(); e.preventDefault(); }
},false);

console.log('[UNKSCAPE] hud_panels.js v1 loaded -- B=hub C=chat wheel=hotbar.');
