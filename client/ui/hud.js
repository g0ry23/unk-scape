...
el.innerHTML=`<div class="hotbar-card" id="hotbar-inner">${slots.map((id,i)=>{const it=US.ITEMS[id]||{name:id,icon:'?'};const equipped=Object.values(p.equipment||{}).includes(id);const qty=g.systems.inventory.count(id)+(equipped?1:0);const selected=g.hotbar.selected===i||(equipped&&['weapon','tool'].includes(it.slot||''));return `<div class="hotbar-slot ${selected?'selected':''} ${qty<=0?'missing':''}" data-slot="${i}" title="${it.name}"><span class="num">${i+1}</span><span class="ico">${it.icon}</span>${qty>1?`<span class="qty">x${qty}</span>`:''}<span class="name">${it.name}</span></div>`;}).join('')}</div>`;
var hc=el.querySelector('#hotbar-inner');if(hc&&!hc._wired){hc._wired=true;hc.addEventListener('click',function(e){var sl=e.target.closest('[data-slot]');if(!sl)return;var idx=parseInt(sl.dataset.slot,10);if(isNaN(idx))return;var gg=window.UnkScape&&window.UnkScape.game;if(!gg)return;gg.hotbar.selected=idx;if(gg.ui&&gg.ui.renderHotbar)gg.ui.renderHotbar();});}
};
