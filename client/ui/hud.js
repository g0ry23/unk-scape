...
root.innerHTML=`<div class="hud-left"><div class="hud-card brand"><div class="brand-mark"></div><div><h1>UNK-SCAPE</h1><small>${g.systems.daynight.label()} - Day ${Math.floor(g.time/g.systems.daynight.dayLength)+1} - ${g.entities.enemies.length} mobs - ${g.camera.modeLabel()}</small></div></div><div class="hud-card bars"><div class="bar-wrap"><span>Health</span><div class="bar hp"><i style="width:${hpPct}%"></i></div><b>${Math.ceil(p.hp)}/${p.maxHp}</b></div><div class="bar-wrap"><span>Combat</span><div class="bar xp"><i style="width:${D.clamp(xpPct,0,100)}%"></i></div><b>Lv ${combatLvl}</b></div></div><div class="hud-card stat-row"><span class="pill gold"><i class="dot"></i>${g.systems.inventory.count('coin')} coins</span><span class="pill green"><i class="dot"></i>Total Lv ${totalLvl}</span><span class="pill purple"><i class="dot"></i>Zoom ${Math.round(g.camera.targetZoom*100)}%</span><span class="pill orange"><i class="dot"></i>Hunger ${g.settings.hungerEnabled?'On':'Off'}</span><span class="pill"><i class="dot"></i>Build ${g.buildMode?'On':'Off'}</span><span class="pill gold"><i class="dot"></i>${D.FACTIONS[p.factionId]?.icon||''} ${p.factionName||'No Faction'}</span><span class="pill purple"><i class="dot"></i>${D.getClassStory(p.classId).title}</span></div></div><div class="hud-right"><div class="hud-card minimap"><canvas id="minimap" width="164" height="164"></canvas></div></div>`;
this.drawMinimap();
};
...
el.innerHTML=`<div class="hotbar-card">${slots.map((id,i)=>{const it=D.ITEMS[id]||{name:id,icon:'?'};const equipped=Object.values(p.equipment||{}).includes(id);const qty=g.systems.inventory.count(id)+(equipped?1:0);const selected=g.hotbar.selected===i||(equipped&&['weapon','tool'].includes(it.slot||''));return `<div class="hotbar-slot ${selected?'selected':''} ${qty<=0?'missing':''}" title="${it.name}"><span class="num">${i+1}</span><span class="ico">${it.icon}</span>${qty>1?`<span class="qty">x${qty}</span>`:''}<span class="name">${it.name}</span></div>`;}).join('')}</div>`;
};
...
ctx.save();ctx.translate(g.player.x/D.TILE*sx,g.player.y/D.TILE*sy);ctx.rotate(g.camera.targetAngle||g.camera.angle||0);ctx.strokeStyle='#f7c65b';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,-9);ctx.lineTo(5,4);ctx.lineTo(0,1);ctx.lineTo(-5,4);ctx.closePath();ctx.stroke();ctx.restore();
};
})();
