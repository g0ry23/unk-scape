...
const US = window.UnkScape = window.UnkScape;
  ...
  US.UI.prototype.renderPanel=function(){
    ...
    el.innerHTML=`<div class="panel-box"><div class="panel-head"><div><h2>${title}</h2><div class="sub">UNK-SCAPE survival interface</div></div><button class="close-btn" onclick="UnkScape.game.ui.closePanel()">Close</button></div><div class="panel-body">${this.panelHTML(this.panel)}</div></div>`;
  };
  ...
  US.UI.prototype.inventoryHTML=function(){
    ...
    const eq=['head','weapon','offhand','body','tool'].map(s=>`<div class="item-card"><h3>${s.toUpperCase()}</h3><p>${p.equipment[s]?US.ITEMS[p.equipment[s]].icon+' '+US.ITEMS[p.equipment[s]].name:'Empty'}</p>${p.equipment[s]?`<button class="small-btn" onclick="event.stopPropagation();UnkScape.game.systems.inventory.unequip('${s}')">Unequip</button>`:''}</div>`).join('');
    const items=Object.entries(inv).map(([id,qty])=>{const it=US.ITEMS[id];return`<div class="item-card"><span class="qty">x${qty}</span><h3>${it.icon} ${it.name}</h3><p>${it.desc||''}</p><span class="tag">${it.type}</span><br><button class="small-btn" onclick="event.stopPropagation();UnkScape.game.systems.inventory.use('${id}')">Use / Equip</button></div>`}).join('')||'<div class="help-note">Inventory empty.</div>';
  ...
  US.UI.prototype.statsHTML=function(){
    ...
    const attrRows=Object.entries(US.ATTRIBUTES).map(([id,a])=>`<div class="stat-line"><span>${a.icon} ${a.name}</span><b>${attr[id]||0} ${(p.attributePoints||0)>0?`<button class="small-btn" style="padding:3px 7px;margin-left:6px" onclick="UnkScape.game.ui.addAttribute('${id}')">+</button>`:''}</b></div>`).join('');
  ...
  US.UI.prototype.bankHTML=function(){
    ...
    const invRows=Object.entries(inv).filter(([id])=>id!=='coin').map(([id,q])=>`<div class="shop-row"><div>${US.ITEMS[id].icon} <b>${US.ITEMS[id].name}</b><small>Backpack x${q}</small></div><button class="small-btn" onclick="event.stopPropagation();UnkScape.game.systems.bank.deposit('${id}',1)">Deposit 1</button><button class="small-btn" onclick="event.stopPropagation();UnkScape.game.systems.bank.deposit('${id}',${q})">All</button></div>`).join('')||'<div class="help-note">No depositable items.</div>';
    const bankRows=Object.entries(bank).map(([id,q])=>`<div class="shop-row"><div>${US.ITEMS[id].icon} <b>${US.ITEMS[id].name}</b><small>Bank x${q}</small></div><button class="small-btn" onclick="event.stopPropagation();UnkScape.game.systems.bank.withdraw('${id}',1)">Withdraw 1</button><button class="small-btn" onclick="event.stopPropagation();UnkScape.game.systems.bank.withdraw('${id}',${q})">All</button></div>`).join('')||'<div class="help-note">Bank empty.</div>';
